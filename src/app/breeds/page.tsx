...
export default function BreedsIndexPage() {
  ...
  // First row of cards is above the fold — eager-load those so one becomes a
  // fast, self-hosted LCP element; everything below the fold stays lazy.
  const dogDisplayOrder = SIZE_ORDER.flatMap((size) =>
    dogs.filter((breed) => breed.size === size),
  );
  const priorityDogSlugs = new Set(
    dogDisplayOrder.slice(0, 4).map((breed) => breed.slug),
  );
  return (
      ...
      <section id="cat" className="scroll-mt-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {cats.map((b) => (
            ...
            >
              <Card padding="sm" className="h-full group-hover:shadow-[0_8px_24px_rgba(42,31,26,0.12)] transition-shadow overflow-hidden p-0">
                <BreedCover petType={b.petType} name={b.name} slug={b.slug} coverUrl={b.coverUrl} />
                <div className="p-4">
